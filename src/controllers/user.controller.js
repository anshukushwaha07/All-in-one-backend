...
export const generateAccessTokenAndRefreshTokens = async (userId) => {
 ...
 const registerUser = asyncHandler(async (req, res) => {
  ...
  const loginUser = asyncHandler(async(req,res)=>{
      // steps of login
      // take email,username and password form body
      // check with database if they existed ot not 
      // if exsist then genrate a refresh token and acess token 
      // send cookies 
      // if not give errors not users existed 
  ...
  const changeCurrentPassword = asyncHandler(async(req,res)=>{
      //1. get old password and new password from body 
      //2. validate them
      //3. get user from db 
      //4. match old password 
      //5. if match then update with new password 
